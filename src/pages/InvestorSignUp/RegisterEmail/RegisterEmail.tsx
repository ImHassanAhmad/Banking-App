...
const SignUp: FC = () => {
  ...
  const { updateActiveStep, registerUser, isLoading, userPayload } = useInvestorSignUpStepper();
  ...
  const onSubmit: SubmitHandler<IForm> = async () => {
    registerUser({
      ...
      onSuccess: () => {
        updateActiveStep(InvestorSignUpFlowSteps.Mobile);
      },
  ...
  return (
    <Stack mt={4}>
      <BackButton
        onClick={() => {
          updateActiveStep(InvestorSignUpFlowSteps.Country);
        }}
      />
      <Stack mt={4}>
        <Heading
